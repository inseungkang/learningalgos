        ...
        if session == 1
            file_list = dir(strcat(file_dir,'AB',num2str(subject),'_Session',num2str(session),'*.mat'));
                for ii = 1:length(file_list)
                    ...
                    viconData.(strcat('AB',num2str(subject))).(strcat('Session',num2str(session))).(strcat('Speed',num2str(rSpeed))).Force_R = foot_r(start_idx:end_idx,:);
                    viconData.(strcat('AB',num2str(subject))).(strcat('Session',num2str(session))).(strcat('Speed',num2str(rSpeed))).Force_L = foot_l(start_idx:end_idx,:);
                end
        end
    end
end
